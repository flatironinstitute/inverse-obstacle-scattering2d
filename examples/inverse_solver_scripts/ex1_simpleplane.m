...
% in this case, the simple plane
%use the function getplane with nterms=20
...
opts = [];
opts.verbose=true;
optim_opts.optim_type = 'sd-min(sd,gn)';
...
dir_data = './plane-data/';
dir_sol = './plane-sol/';
dir_diary = './plane-diary/';


fname = [dir_data 'simpleplane_ik' num2str(k0) '_nk' int2str(nk) '_dk' ...
     num2str(dk) '_inctype' int2str(inc_type) ...
     '_noise' int2str(noise_type) 'noise_lvl' num2str(noise_lvl) ... 
     '_data_' bc.type '.mat'];

fname_sol = [dir_sol 'simpleplane_ik' num2str(k0) '_nk' int2str(nk) '_dk' ...
     num2str(dk) '_inctype' int2str(inc_type) ...
     '_noise' int2str(noise_type) 'noise_lvl' num2str(noise_lvl) ... 
...
fname_diary = [dir_diary 'simpleplane_ik' num2str(k0) '_nk' int2str(nk) '_dk' ...
     num2str(dk) '_inctype' int2str(inc_type) ...
     '_noise' int2str(noise_type) 'noise_lvl' num2str(noise_lvl) ... 
     '_data_' bc.type '_optimtype_' optim_opts.optim_type '_filtertype_' ...
     optim_opts.filter_type '_ifcons' int2str(ifcons) '_ncurvmin' ...
...
%     src_info = geometries.get_plane(n,70);
    src_info = geometries.get_plane(n,20);
...
fname_result = [dir_data 'simpleplane_ik' num2str(k0) '_nk' int2str(nk) '_dk' ...
     num2str(dk) '_inctype' int2str(inc_type) ...
     '_noise' int2str(noise_type) 'noise_lvl' num2str(noise_lvl) ... 
     '_data_' bc.type '_optimtype_' optim_opts.optim_type '_filtertype_' ...
     optim_opts.filter_type '_ifcons' int2str(ifcons) '_ncurvmin' ...
...
fac = 1;
src_init=[];
src_init = geometries.ellipse(1.0,1.0,500);
...
% save(fname_result,'allxs','allys','errs','ik_lists','reso','resa');
% save(fname_result,'inv_data_all','allxs','allys','errs','ik_lists','reso','resa','-v7.3');
save(fname_sol,'-v7.3')
