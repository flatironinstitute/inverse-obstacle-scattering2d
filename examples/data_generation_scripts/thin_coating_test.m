...
narms2 = 10;
amp2 = 0.03;
chnkr2 = chunkerfunc(@(t) starfish(t, narms2, amp2, [], [], 0.94), cparams, pref); 
...
%% Plane wave data + post processing

thet = rand*2*pi;
dir = [cos(thet); sin(thet)];
[u, dudn] = get_planewave_data(zk1, chnkr1, dir);

n1 = 2*chnkr1.npt;
ntot = n1 + chnkr2.npt;
rhs = complex(zeros(ntot,1));
rhs(1:2:n1) = -u;
rhs(2:2:n1) = -dudn;
...
ntarg = 100;
rtarg = 10;
thet_targ = 2*pi/ntarg:2*pi/ntarg:2*pi;
targs = rtarg*[cos(thet_targ); sin(thet_targ)];
...
targinfo1 = [];
targinfo1.r = targs;
usc = chunkerkerneval(chnkr1, K1, sol(1:n1), targinfo1);
uin = get_planewave_data_targ(zk1, targinfo1, dir);
utot = uin + usc;


function u = get_planewave_data_targ(zk1, targs, dir) 
    dirnorm = norm(dir(:));
    diruse = dir/dirnorm;
    if isa(targs, 'struct')
        r = targs.r;
    ...
    zexp = exp(1j*zk1*(diruse(1)*r(1,:) + diruse(2)*r(2,:))).';
    u = zexp;
end


function [u, dudn] = get_planewave_data(zk1, chnkr1, dir)
    dirnorm = norm(dir(:));
    diruse = dir/dirnorm;
